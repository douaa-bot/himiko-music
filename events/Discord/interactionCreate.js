...
module.exports = async (client, inter) => {
    ...
    if (inter.type === InteractionType.ApplicationCommand) {
        ...
        if (command.voiceChannel) {
            // Vérifie si 'voice' existe et si 'voice.channel' est bien défini
            ...
            // Vérifie si le bot est dans un canal vocal et si l'utilisateur est dans le même canal
        ...
        const allowedUserIds = ['1005276651485139034']; // Remplace avec les vrais IDs
        if (!allowedUserIds.includes(inter.user.id)) {
            errorEmbed.setDescription(await Translate(`<❌> | You don't have permission to execute this command.`));
            return inter.editReply({ embeds: [errorEmbed], ephemeral: true });
        }
